import { createNotification, getAllNotifications, getNotificationsByEmployee, markNotificationAsRead, } from "../services/notification.service.js";
...
export async function store(req, res) {
    try {
        ...
        if (!employeeId ||
            !title ||
            !message) {
            return errorResponse(res, 400, "employeeId, title and message are required");
        }
...
export async function myNotifications(req, res) {
    try {
        ...
        const notifications = await getNotificationsByEmployee(BigInt(employeeId));
        return successResponse(res, 200, "Notifications retrieved successfully", notifications);
    }
    catch (error) {
...
export async function markAsRead(req, res) {
    try {
        ...
        const result = await markNotificationAsRead(id, BigInt(employeeId));
        if (result.count === 0) {
