import { getEmployeeProfile, updateEmployeeProfile, } from "../services/profile.service.js";
...
export async function myProfile(req, res) {
    try {
        ...
        const profile = await getEmployeeProfile(BigInt(employeeId));
        if (!profile) {
        ...
        return successResponse(res, 200, "Profile retrieved successfully", {
            ...
            email: profile.user?.email ??
                profile.email,
            phone: profile.phone ?? "",
            location: profile.office.address ??
                profile.office.name,
        });
    }
    catch (error) {
...
export async function updateMyProfile(req, res) {
    try {
        ...
        const lastName = nameParts.length > 0
            ? nameParts.join(" ")
            : null;
        const updated = await updateEmployeeProfile(BigInt(employeeId), {
            firstName,
            lastName,
            email: email.trim(),
        ...
        return successResponse(res, 200, "Profile updated successfully", {
            ...
            email: updated.user?.email ??
                updated.email,
            phone: updated.phone ?? "",
            location: updated.office.address ??
                updated.office.name,
        });
    }
    catch (error) {
