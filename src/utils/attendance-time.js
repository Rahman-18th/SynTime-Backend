export function getShiftStartDateTime(workDate, shiftStartTime) {
    const timezoneOffsetMinutes = Number(process.env.TIMEZONE_OFFSET_MINUTES ?? 420);
    /*
     ...
     */
    const shiftAsUtc = Date.UTC(year, month, day, hours, minutes, seconds);
    /*
     ...
     * 08:00 WIB = 01:00 UTC
     */
    return new Date(shiftAsUtc -
        timezoneOffsetMinutes * 60 * 1000);
}
export function determineAttendanceStatus(checkInAt, workDate, shiftStartTime) {
    const toleranceMinutes = Number(process.env.LATE_TOLERANCE_MINUTES ?? 10);
    const shiftStart = getShiftStartDateTime(workDate, shiftStartTime);
    const lateThreshold = new Date(shiftStart.getTime() +
        toleranceMinutes * 60 * 1000);
    return checkInAt.getTime() <= lateThreshold.getTime()
        ? "present"
        : "late";
}
export function isScheduleToday(workDate) {
    const timezoneOffsetMinutes = Number(process.env.TIMEZONE_OFFSET_MINUTES ?? 420);
    const now = new Date();
    const localNow = new Date(now.getTime() +
        timezoneOffsetMinutes * 60 * 1000);
    return (workDate.getUTCFullYear() ===
...
//# sourceMappingURL=attendance-time.js.map